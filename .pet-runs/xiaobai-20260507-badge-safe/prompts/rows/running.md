Create a single horizontal sprite strip for the Codex app digital pet `snowball` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: round fluffy white cat, yellow-green eyes, tiny pink nose, upright begging pose, cute chibi codex pet.
