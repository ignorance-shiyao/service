...
import {
  ...
  Cpu, Send, ChevronRight, ArrowRight, Link as LinkIcon,
} from 'lucide-react';
...
export const FaultReportingView: React.FC<FaultReportingProps> = ({ mode, onToggleMode, onClose }) => {
    ...
    const mockResults: AssetDiagnostic[] = [
      {
        ...
        overallStatus: 'fault',
        suggestion: '您的专线出现持续丢包，通常意味着线路硬件需要现场处理，建议立即报障。',
        metrics: [
      ...
      },
      {
        ...
        uuid: 'SDWAN-GW-SH-02',
        overallStatus: 'minor',
        suggestion: '当前仅出现轻度波动，建议加入观察并持续跟踪。',
        metrics: [
  ...
  const handleGoToReport = (asset: AssetDiagnostic) => {
    ...
    setTicketMemo(`【一键诊断报障】您的「${asset.name}」当前${asset.overallStatus === 'fault' ? '出现严重故障' : '存在明显异常'}。建议：${asset.suggestion || '请尽快联系客户经理处理。'}`);
    setStep('form');
  };
  ...
  const submitTicket = () => {
    ...
    setTimeout(() => {
      ...
      showAppToast(`工单已受理（编号 ${ticketId}），已自动定位到「${selectedAsset?.name || '-'}」，预计 30 分钟内回复进展。`, {
        title: '报障已受理',
        tone: 'success',
        duration: 4200,
      });
  ...
  const statusValueClass = (status: DiagnosticStatus) => {
    if (status === 'fault') return 'text-red-400';
    if (status === 'major' || status === 'warning') return 'text-amber-400';
    if (status === 'minor') return 'text-sky-300';
    return 'text-slate-200';
  };

  const statusBarClass = (status: DiagnosticStatus) => {
    if (status === 'fault') return 'bg-red-500';
    if (status === 'major' || status === 'warning') return 'bg-amber-500';
    if (status === 'minor') return 'bg-sky-400';
          ...
          <div className="h-full flex flex-col items-center justify-center p-10 text-center space-y-10 animate-in fade-in zoom-in duration-500">
            ...
            <div className="max-w-lg">
              ...
              <p className="text-slate-400 leading-relaxed">
                针对专线时延丢包、5G终端信号、智算节点存活及动环越限进行自动扫描。发现异常后可直接发起报障，我们会自动附带诊断结论，减少重复沟通。
              </p>
            </div>
            <Button onClick={startDetection} className="bg-blue-600 hover:bg-blue-500 h-16 px-12 rounded-2xl font-black text-xl shadow-2xl shadow-blue-900/40 border-none group">
              立即检查我的业务
        ...
        {step === 'scanning' && (
          <div className="h-full flex flex-col items-center justify-center p-10 animate-in fade-in duration-300">
            <div className="relative mb-10">
              ...
              <div className="absolute inset-0 flex items-center justify-center text-lg font-bold font-mono text-blue-400">{scanProgress}%</div>
            </div>
                    ...
                    <div className="flex gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <Badge color={statusBadgeColor(asset.overallStatus)} className="scale-75 origin-left">{statusText(asset.overallStatus)}</Badge>
                        </div>
                      </div>
                    </div>
                    {asset.overallStatus === 'fault' && (
                      <Button onClick={() => handleGoToReport(asset)} className="bg-red-600 hover:bg-red-500 border-none h-10 px-4 text-xs font-bold shadow-lg shadow-red-900/40">
                        我要解决这个问题
                      </Button>
                    )}
                    ...
                    {asset.metrics.map((m, idx) => (
                      <div key={idx} className="bg-slate-800/40 rounded-2xl p-4 border border-slate-700/50 flex flex-col justify-between h-24">
                        ...
                        <div className="flex items-baseline gap-1 mt-1">
                          <span className={`text-xl font-black font-mono ${statusValueClass(m.status)}`}>{m.value}</span>
                        ...
                        <div className="h-1 w-full rounded-full mt-2 bg-slate-700 overflow-hidden">
                          <div className={`h-full rounded-full ${statusBarClass(m.status)}`} style={{ width: '65%' }}></div>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {asset.suggestion && (
                    <div className={`mt-4 rounded-2xl border p-4 ${asset.overallStatus === 'fault' ? 'bg-red-950/20 border-red-900/50 text-red-200/80' : 'bg-slate-800/50 border-slate-700 text-slate-400'}`}>
                      <div className="mb-2 text-xs font-semibold">
                        {asset.overallStatus === 'fault' ? '业务受影响，建议立即处理。' : '存在明显异常，建议尽快关注。'}
                      </div>
            ...
            <div className="max-w-3xl mx-auto w-full space-y-10">
              ...
              <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-8 space-y-8 shadow-2xl relative overflow-hidden">
                ...
                <div className="pt-6 border-t border-slate-800 flex justify-end gap-4">
                  ...
                  <Button onClick={submitTicket} className="bg-blue-600 hover:bg-blue-500 h-12 px-10 rounded-xl font-bold shadow-lg shadow-blue-900/40 border-none flex items-center gap-2">
                    <Send size={16} />
                    确认提交工单
                  </Button>
                </div>
